...
class Phase1(RunBase):
    def __init__(self):
        ...
        self.lr_policy = LinearIterationPolicy(0, 0, 3000, 1.2e-4)

        self.train_augs = [RandomResizedCropWithProb(size=self.crop_size, probability=0.85),
                           RandomFlip(), RotateWithProb(probability=0.85)]
        self.val_augs = None
