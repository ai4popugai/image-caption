...
def create_dataset(experiment: str, run: str, phase: str, snapshot_name: str, dataset_dir: str, name_prefix: str = '',
    ...
    emd_dir = os.path.join(dst_dir, FEATURE_MAP_KEYS)
    ...
    with torch.no_grad():
        for idx, ff in enumerate(dataset.frames_list):
            ...
            # get batch
            batch = dataset[idx]
            batch[FRAME_KEY] = batch[FRAME_KEY].unsqueeze(0)
            batch[LABEL_KEY] = batch[LABEL_KEY].unsqueeze(0)
            ...
            # inference
            result = model(batch)
            torch.save(result[FEATURE_MAP_KEYS].squeeze(), os.path.join(emd_dir, f'{id}.pth'))
